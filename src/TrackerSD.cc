                                     ...
                                     G4TouchableHistory*)
{
  ...
  auto myEvent = (EventAction*)(runManager->GetUserEventAction());
  G4int currentLayer = myEvent->GetCurrentLayer();

  G4int hitLayer = aStep->GetPreStepPoint()->GetTouchableHandle()
                                               ->GetCopyNumber();

  if (currentLayer == hitLayer) return false;
  else
  {
    myEvent->SetCurrentLayer(hitLayer);
    currentLayer = myEvent->GetCurrentLayer();
  }
  // energy deposit
